  ...
  return (
    <Link to={`/homepage/${friend.id}`}>
      <div className="flex flex-col justify-center items-center bg-white drop-shadow-md py-6 rounded-lg">
        ...
        <h2 className="text-[20px] text-[#1F2937] font-semibold mb-3">
          {name}
        </h2>
        ...
        <div className="flex gap-2 mb-2">
          {tags.map((tag, index) => {
            return (
              <button
                key={index}
                className="bg-[#CBFADB] px-4 py-2 rounded-full"
              >
                {capitalWords(tag)}
              </button>
            );
        ...
        <div>
          <button
            className={`px-3 py-2 text-[12px] font-medium rounded-full text-white
                ${statusStyle[status]}`}
          >
            {capitalWords(status)}
          </button>
